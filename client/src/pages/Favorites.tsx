...
import { ArrowLeft, Loader2, Heart, TestTube } from "lucide-react";
      ...
      <div className="min-h-screen bg-background">
        <header className="bg-background border-b sticky top-0 z-50">
          <div className="container flex items-center h-14">
            ...
            <h1 className="font-semibold ml-2">我的收藏</h1>
          </div>
        </header>
        <div className="flex flex-col items-center justify-center p-8 mt-20">
          <Heart className="h-12 w-12 text-muted-foreground mb-4" />
          <p className="text-muted-foreground mb-4">请登录后查看收藏</p>
          <Button asChild>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="bg-background border-b sticky top-0 z-50">
        <div className="container flex items-center justify-between h-14">
          ...
          <span className="text-sm text-muted-foreground">
            {favoritesQuery.data?.length || 0} 个项目
          </span>
        </div>
      </header>
      ...
      <main className="container py-4">
        ...
        {!favoritesQuery.isLoading && (!favoritesQuery.data || favoritesQuery.data.length === 0) && (
          <div className="text-center py-12">
            <Heart className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
            <p className="text-muted-foreground mb-4">暂无收藏项目</p>
